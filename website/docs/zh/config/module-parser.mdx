...
用于控制 Rspack 是否解析 `import.meta.webpackContext()` 调用。当同时配置了 `javascript.importMeta.webpackContext` 和 `javascript.importMetaContext` 时，`javascript.importMeta.webpackContext` 的优先级更高。
