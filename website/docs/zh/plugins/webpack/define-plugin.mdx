...
通常你不需要手写这个 `DefinePlugin`。[`EnvironmentPlugin`](/plugins/webpack/environment-plugin) 以及用户 `DefinePlugin` 中的 `import.meta.env.*` / `process.env.*` 键，都会被收集到同一个累积的环境对象中，并自动以 `import.meta.env` 暴露。
