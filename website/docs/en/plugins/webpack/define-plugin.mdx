...
You usually do not write this `DefinePlugin` by hand. [`EnvironmentPlugin`](/plugins/webpack/environment-plugin) and any user `DefinePlugin` `import.meta.env.*` / `process.env.*` keys are all collected into one accumulated env object and exposed as `import.meta.env` automatically.
...
When the env object is accumulated (via [`EnvironmentPlugin`](/plugins/webpack/environment-plugin) / `NODE_ENV`), each of its keys is also mirrored onto `process.env.KEY` per-key, so the same value is reachable from both `import.meta.env.KEY` and `process.env.KEY` — without replacing the whole `process.env` object.
